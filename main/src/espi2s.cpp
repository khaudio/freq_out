...
{
    write_bytes(reinterpret_cast<const void*>(&((*data)[0])), length * sizeof(T));
}
...
{
    #if _DEBUG
    read_bytes(reinterpret_cast<void*>(&(data->at(0))), length * sizeof(T));
    #else
    read_bytes(reinterpret_cast<void*>(&((*data)[0])), length * sizeof(T));
    #endif
}
